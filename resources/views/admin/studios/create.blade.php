        ...
        <div class="card shadow">
            <div class="card-header">
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">{{ __('create studio') }}</h1>
            ...
            <div class="card-body">
                <form action="{{ route('admin.studios.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label for="names">{{ __('Nama studio') }}</label>
                        <input type="text" class="form-control" id="names" placeholder="{{ __('names') }}" name="names" value="{{ old('names') }}" />
                    </div>
                    <div class="form-group">
...
<script>
    Dropzone.options.photoDropzone = {
        ...
        init: function () {
            @if(isset($studios) && $studios->photo)
                var file = {!! json_encode($studios->photo) !!}
                    this.options.addedfile.call(this, file)
                this.options.thumbnail.call(this, file, "{{ $studios->photo->getUrl('thumb') }}")
