        ...
        <div class="card shadow">
            <div class="card-header">
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">{{ __('create booking Paket') }}</h1>
                    <a href="{{ route('admin.bookingpaket.index') }}" class="btn btn-primary btn-sm shadow-sm">{{ __('Go Back') }}</a>
                </div>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.bookingpaket.store') }}" method="POST">
                    @csrf
                    <div class="form-group">
                        <label for="services_id">{{ __('Jenis Paket') }}</label>
                        <select name="services_id" id="services_id" class="form-control">
                            @foreach($services as $service)
                                <option {{ $servicesString == $service->name ? 'selected' : null }} value="{{ $service->id }}">{{ $service->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
