    ...
    <div class="card shadow">
        <div class="card-header">
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
                ...
                <a href="{{ route('admin.bookingpaket.index') }}"
            ...
            <form action="{{ url('admin/bookingpaket/'. $bookingpaket->id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" disabled class="form-control" id="name" name="name"
                        value="{{ old('name', $bookingpaket->user->name) }}" />
                </div>
                <div class="form-group">
                    ...
                    <select name="services_id" id="services_id" class="form-control">
                        ...
                        <option disabled selected
                            {{ $bookingpaket->services->name == $service->string ? 'selected' : null }}
                ...
                <div class="form-group">
                    ...
                    <input type="text" disabled class="form-control datetimepicker" id="time_from" name="time_from"
                        value="{{ old('time_from', $bookingpaket->time_from) }}" />
                </div>
                <div class="form-group">
                    <label for="time_to">{{ __('Jam Selesai') }}</label>
                    <input type="text" disabled class="form-control datetimepicker" id="time_to" name="time_to"
                        value="{{ old('time_to', $bookingpaket->time_to) }}" />
                </div>
                <div class="form-group">
                    <label for="status">{{ __('Status') }}</label>
                    <select name="status" id="status" class="form-control">
                        <option {{ $bookingpaket->status == 'On Proses' ? 'selected' : null }} value="0">Menunggu
                            konfirmasi</option>
